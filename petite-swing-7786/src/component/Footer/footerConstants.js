export const footProduct = [
  "Proposals",
  "Contracts",
  "Invoicing",
  "Client CRM",
  "Time Tracking",
  "Task Tracking",
  "Forms",
  "Accounting",
  "Bonsai Tax",
  "Bonsai Cash",
  "Pricing",
  "Bonsai Reviews",
];

export const footFreeResources1 = [
  "Freelance Resources",
  "Freelance Blog by Bonsai",
  "How to Write a Contract",
  "Online Signature Maker",
];

export const footFreeResources2 = [
  "Self-Employed Taxes Hub",
  "Self-Employed Tax Calculator",
  "Self-Employed Tax Deductions",
];

export const footTemplates = [
  {
    head: "Invoice Templates",
    someElse: [
      "Invoice Generator",
      "Blank Invoice Template",
      "Consultant Invoice Template",
      "Editable Invoice Template",
      "Graphic Design Invoice",
      "Web Design Invoice",
      "Contractor Invoice Template",
      "Photography Invoice Template",
      "Self Employed Invoice Template",
    ],
  },
  {
    head: "Proposal Templates",
    someElse: [
      "Website Proposal Template",
      "Digital Marketing Proposal Template",
      "Social Media Marketing Proposal",
      "Graphic Design Proposal",
      "Freelance Proposal Template",
      "SEO Proposal Template",
    ],
  },
  {
    head: "Contract Templates",
    someElse: [
      "Online Contract",
      "Graphic Design Contract",
      "Freelance Contract Template",
      "Social Media Marketing Contract",
      "Software Development Contract",
      "Web Design Contract",
      "Video Production Contract",
      "Public Relations Contract",
      "Translation Contract",
    ],
  },
  {
    head: "Agreement Templates",
    someElse: [
      "Retainer Agreement Template",
      "Influencer Agreement Template",
      "Collaboration Agreement Template",
      "Software Development Agreement",
      "Marketing Agreement Template",
    ],
  },
  {
    head: "Scope of Work Templates",
    someElse: [
      "Website Development Scope of Work",
      "Digital Marketing Scope of Work",
      "Consultant Statement of Work Template",
      "Software Development Scope of Work",
    ],
  },
  {
    head: "Quote Templates",
    someElse: [
      "Freelance Quotation Template",
      "Web Design Quotation Template",
      "Graphic Design Quotation Template",
      "Interior Design Quotation Template",
      "Video Production Quotation Template",
      "Social Media Marketing Quotation",
    ],
  },
  // {
  //   head: "Credit Note Templates",
  // },
  // {
  //   head: "Estimate Templates",
  // },
];

export const footBonsai = [
  "About",
  "Careers",
  "Support",
  "LinkedIn",
  "Twitter",
  "Privacy policy",
  "Legal",
  "Affiliates",
  "Write for Us",
];
